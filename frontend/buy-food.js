...
async function searchRestaurants() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      resultsDiv.innerHTML =
        '<div class="no-results"><h3>No restaurants found</h3><p>Try searching with different keywords</p></div>';
    }
  } catch (error) {
    resultsDiv.innerHTML =
      '<div class="no-results"><h3>Error searching restaurants</h3><p>Please try again later.</p></div>';
  }
}
...
async function loadAllRestaurants() {
  const resultsDiv = document.getElementById("restaurantResults");
  resultsDiv.innerHTML =
    '<div class="no-results"><h3>Loading restaurants...</h3></div>';
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (restaurants.length === 0) {
        resultsDiv.innerHTML =
          '<div class="no-results"><h3>No restaurants found.</h3></div>';
      } else {
      ...
      resultsDiv.innerHTML =
        '<div class="no-results"><h3>Error loading restaurants.</h3></div>';
    }
  } catch (error) {
    resultsDiv.innerHTML =
      '<div class="no-results"><h3>Error loading restaurants.</h3></div>';
...
function updateRatingText() {
  ...
  ratingText.textContent =
    selectedRating > 0 ? ratingLabels[selectedRating] : "Select rating";
}
